...
def run_cycle_15m(manual=False):
    ...
    if not lock.acquire(blocking=False):
        debug_mark(bot_15m, cfg.get("chat_id_15m"), 901, "run_cycle_15m_lock_busy")
        return
    start_time = now_utc()
    try:
        symbols = cfg["symbols_15m"]
        chat_id = cfg.get("chat_id_15m") or cfg.get("chat_id_1h")
        if chat_id and bot_15m:
            bot_15m.send_message(chat_id, f"شروع سیکل 15m برای {len(symbols)} نماد...\n{now_utc_str()}")
        batch = cfg.get("cycle_progress_batch", 5)
        for idx, sym in enumerate(symbols, start=1):
            info = create_plotly_chart(sym, "15m", cfg["lookback_15m"], cfg["max_bars"], f"15m_{sym}.png")
            alarms = detect_alarms(cfg, info, "15m")
            if chat_id and bot_15m:
                if cfg["verbose_15m"] or alarms:
                    try:
                        with open(info["png_path"], "rb") as img:
                            bot_15m.send_photo(chat_id, img, caption=f"{sym} – 15m\n{now_utc_str()}")
                    except Exception:
                        debug_mark(bot_15m, chat_id, 902, "run_cycle_15m_send_photo")
                if alarms:
                    txt = f"آلارم 15m برای {sym}:\n" + "\n".join(f"- {a}" for a in alarms)
                    bot_15m.send_message(chat_id, txt)
            if idx % batch == 0 and chat_id and bot_15m:
                bot_15m.send_message(chat_id, f"پیشرفت 15m: {idx}/{len(symbols)} نماد")
        elapsed = (now_utc() - start_time).total_seconds()
        if elapsed < cfg.get("cycle_min_duration_sec", 5):
            debug_mark(bot_15m, chat_id, 903, "run_cycle_15m_too_fast")
        if chat_id and bot_15m:
            bot_15m.send_message(chat_id, f"پایان سیکل 15m.\nمدت زمان: {int(elapsed)} ثانیه")
    except Exception:
        debug_mark(bot_15m, cfg.get("chat_id_15m"), 904, "run_cycle_15m_error")
    finally:
        lock.release()
...
def run_now_15m(m):
    try:
        bot_1h.send_message(m.chat.id, "اجرای فوری سیکل 15m آغاز شد...")
    except Exception:
...
def run_all_cycles(m):
    try:
        bot_1h.send_message(m.chat.id, "اجرای چرخه‌ی 15m آغاز شد...")
    except Exception:
...
def main():
    if bot_15m:
        t = threading.Thread(target=scheduler_15m, daemon=True)
        t.start()
    try:
        if bot_1h:
            bot_1h.infinity_polling()
        elif bot_15m:
            bot_15m.infinity_polling()
    except Exception:
        debug_mark(bot_1h, None, 908, "main_polling_error")
